#include <vector>

using namespace std;

// функция шифрования и дешифрования. они отличаются только порядком передачи ключей в 16 шагах алгоритма
vector<unsigned char> des(vector<unsigned char> data, vector<unsigned char> key8, bool encode);

// получение 16 ключей по 48 бит на основе 64 битного ключа шифрования
vector<vector<unsigned char> > getKeys(vector<unsigned char> key64);

// один шаг алгоритма DES:
// перастановка правой части с расширением
// подстановка в 8 S-блоках
// перестановка
// сложение по модулю 2 правой и левой частей
void step(vector<unsigned char>& right, vector<unsigned char>& left, vector<unsigned char> key48);

// подстановки с помощью 8 S-блоков
vector<unsigned char> substitution(vector<unsigned char> right48);




vector<unsigned char> des(vector<unsigned char> data, vector<unsigned char> key8, bool encode)
{
	// переведём 8 байт в 64 бита
	// для данных и ключа
	vector<unsigned char> bits;
	vector<unsigned char> key64;
	for (int i = 0; i < 8; i++)
	{
		unsigned char b = data[i];
		unsigned char k = key8[i];
		for (int j = 0; j < 8; j++)
		{
			bits.push_back(b % 2);
			b = b / 2;

			key64.push_back(k % 2);
			k = k / 2;
		}
	}

	// разобьём биты данных на 2 половинки
	vector<unsigned char> left, right;
	left.insert(left.end(), bits.begin(), bits.begin() + 32);
	right.insert(right.end(), bits.begin() + 32, bits.end());

	// сгенерируем 16 48-битных ключей на основе одного 64-битного
	vector<vector<unsigned char> > keys = getKeys(key64);

	// основные 16 шагов алгоритма
	// при шифровании и дешифровании различается только порядок передачи ключей
	for (int i = 0; i < 16; i++)
	{
		if (encode)
			step(right, left, keys[i]);
		else
			step(right, left, keys[15 - i]);
	}

	// объединяем левую и правые части (учитывая, что на последнем шаге они не должны были менться местами
	bits.clear();
	bits.insert(bits.end(), right.begin(), right.end());
	bits.insert(bits.end(), left.begin(), left.end());


	// теперь массив бит превращаем обратно в массив байт
	vector<unsigned char> result;
	for (int i = 0; i < 8; i++)
	{
		unsigned char b = 0;
		unsigned char pow2 = 1; // степень 2, на которую необходимо умножать очередной бит
		for (int j = 0; j < 8; j++)
		{
			b += bits[i * 8 + j] * pow2;
			pow2 *= 2;
		}
		result.push_back(b);
	}

	return result;
}

vector<vector<unsigned char> > getKeys(vector<unsigned char> key64)
{
	// правило выбора 56 бит из 64
	int r1[] = {
		57, 49, 41, 33, 25, 17, 9, 1,
		58, 50, 42, 34, 26, 18, 10, 2,
		59, 51, 43, 35, 27, 19, 11, 3,
		60, 52, 44, 36, 63, 55, 47, 39,
		31, 23, 15, 7, 62, 54, 46, 38,
		30, 22, 14, 6, 61, 53, 45, 37,
		29, 21, 13, 5, 28, 20, 12, 4
	};

	// берём необходимые 56 бит из 64
	vector<unsigned char> key56(56);
	for (int i = 0; i < 56; i++)
	{
		key56[i] = key64[r1[i] - 1];
	}

	// сколько раз надо сдвигать 56 битный ключ
	// для получения очередного 48 битного
	int shs[] = { 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 };

	// правило выбора 48 бит из 56
	unsigned char r2[] = {
		14, 17, 11, 24, 1, 5, 3, 28,
		15, 6, 21, 10, 23, 19, 12, 4,
		26, 8, 16, 7, 27, 20, 13, 2,
		41, 52, 31, 37, 47, 55, 30, 40,
		51, 45, 33, 48, 44, 49, 39, 56,
		34, 53, 46, 42, 50, 36, 29, 32
	};
	
	vector<vector<unsigned char> > keys;
	for (int j = 0; j < 16; j++)
	{
		// сдвигаем половинки ключа 
		while (shs[j] > 0)
		{
			unsigned char temp1 = key56[0];
			unsigned char temp2 = key56[28];

			for (int i = 0; i < 55; i++)
			{
				if (i == 27)
					continue;
				else
					key56[i] = key56[i + 1];
			}
			key56[27] = temp1;
			key56[55] = temp2;
			shs[j] -= 1;
		}

		// собираем новый 48 битный ключ
		vector<unsigned char> key48(48);
		for (int i = 0; i < 48; i++)
		{
			key48[i] = key56[r2[i] - 1];
		}
		keys.push_back(key48);
	}
	return keys;
}

void step(vector<unsigned char>& right, vector<unsigned char>& left, vector<unsigned char> key48)
{
	// переставляем биты правой части так, чтобы из 32 бит стало 48
	vector<unsigned char> right48(48);
	int E[] = {
		32, 1, 2, 3, 4, 5, 4, 5,
		6, 7, 8, 9, 8, 9, 10, 11,
		12, 13, 12, 13, 14, 15, 16, 17,
		16, 17, 18, 19, 20, 21, 20, 21,
		22, 23, 24, 25, 24, 25, 26, 27,
		28, 29, 28, 29, 30, 31, 32, 1
	};
	for (int i = 0; i < 48; i++)
	{
		right48[i] = right[E[i] - 1];
	}



	// складываем по модулю 2 правую часть с ключом
	for (int i = 0; i < 48; i++)
	{
		right48[i] = right48[i] ^ key48[i];
	}



	// посдтановка с помощью S-блоков
	// из 48 бит правой части получаем 32
	vector<unsigned char>  newRight32 = substitution(right48);

	

	// простая перестановка
	unsigned char per[] = {
		16, 7, 20, 21, 29, 12, 28, 17,
		1, 15, 23, 26, 5, 18, 31, 10,
		2, 8, 24, 14, 32, 27, 3, 9,
		19, 13, 30, 6, 22, 11, 4, 25
	};

	vector<unsigned char>  temp(32);
	for (int i = 0; i < 32; i++)
		temp[i] = newRight32[per[i] - 1];
	newRight32 = temp;



	// сложение по модулю 2 новой правой и старой левой частей
	for (int i = 0; i < 32; i++)
		newRight32[i] = newRight32[i] ^ left[i];



	// после этого новый левый блок - старый правый
	left = right;
	right = newRight32;
}

// подстановки с помощью 8 S-блоков
vector<unsigned char> substitution(vector<unsigned char> right48)
{
	int  sboxes[8][4][16] = {
		14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7,
		0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8,
		4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0,
		15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13,

		15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10,
		3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5,
		0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15,
		13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9,

		10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8,
		13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1,
		13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7,
		1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12,

		7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15,
		13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9,
		10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4,
		3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14,

		2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9,
		14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6,
		4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14,
		11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3,

		12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11,
		10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 12, 14, 0, 11, 3, 8,
		9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6,
		4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13,

		4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1,
		13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6,
		1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2,
		6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12,

		13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7,
		1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2,
		7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3, 5, 8,
		2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11
	};

	// разбиваем входные 48 бит на массивы по 6 бит
	unsigned char right8x6[8][6];
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 6; j++)
		{
			right8x6[i][j] = right48[i * 6 + j];
		}
	}

	// преобразуем 8 частей по 6 бит в одну 32-битную 
	// по S-блокам
	vector<unsigned char> newRight32;
	for (int i = 0; i < 8; i++)
	{
		// биты 1 и 6 - номер строки таблицы
		int row = right8x6[i][0] * 2 + right8x6[i][5];

		// биты со 2 по 5 - номер столбца
		int column =
			right8x6[i][1] * 8
			+ right8x6[i][2] * 4
			+ right8x6[i][3] * 2
			+ right8x6[i][4] * 1;

		// получаем значение из таблиц
		int sValue = sboxes[i][row][column];

		// потом полученное значение переводим в двоичный вид
		int sBits[4];
		for (int j = 4; j; j--)
		{
			sBits[j - 1] = sValue % 2;
			sValue /= 2;
		}

		// записываем полученное значение к результату
		for (int j = 0; j < 4; j++)
		{
			newRight32.push_back(sBits[j]);
		}
	}
	return newRight32;
}